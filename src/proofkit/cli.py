...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Capture environment variables (name-pattern redacted).",
)
...
@click.option(
    ...
    help="Directory to replay the command in (default: current directory).",
)
@click.option(
    "--allow-same-commit",
    is_flag=True,
    default=False,
    help="Skip the warning when replaying against the exact same git commit that was captured.",
)
