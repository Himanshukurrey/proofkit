...
- **Windows: works, with one known sharp edge, and it hasn't been tested on a real Windows machine.** ProofKit deliberately never uses `shell=True` (see "Why," above — it's what keeps quoting behavior identical across machines). The tradeoff: on Windows, tools that ship as `.cmd`/`.bat` wrapper scripts — `npm`, `yarn`, `pnpm`, and by extension most Node-based CLIs (`tsc`, `eslint`, `jest`, etc. in `node_modules/.bin`) — can't be launched directly that way; Windows requires a command interpreter to run them, which is a documented Python `subprocess` limitation, not something specific to this tool. If `capture`/`verify` fails to launch such a command, ProofKit detects this specific case and prints a hint with the workaround: run it through cmd explicitly, e.g. `proofkit capture -- cmd /c npm test`. This logic is reasoned through from documented Windows/Python behavior and unit-tested with mocks (see `tests/test_diagnostics.py`), but **has not been verified on an actual Windows machine** — if you hit something different, please open an issue.
- **The Claude Code plugin's tool pre-approval may not apply on native Windows.** The bundled skill declares `allowed-tools: Bash(proofkit *)` so Claude Code can run `proofkit` without a permission prompt each time. On native Windows without Git Bash installed, Claude Code uses a separate PowerShell tool instead of the Bash tool — whether the same pre-approval syntax applies there is untested. Worst case, Windows users see an extra permission prompt per `proofkit` call; the skill still works either way.
