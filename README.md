...
- **The Claude Code plugin's tool pre-approval on native Windows is still untested from inside an actual Claude Code permission prompt.** The bundled skill declares `allowed-tools: Bash(proofkit *)` so Claude Code can run `proofkit` without a permission prompt each time. This targets Claude Code's Bash tool specifically; on native Windows without Git Bash installed, Claude Code uses a separate PowerShell tool instead, and whether the same pre-approval syntax applies there hasn't been confirmed. Worst case, Windows users see an extra permission prompt per `proofkit` call; the skill still works either way.
