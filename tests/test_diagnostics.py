...
def test_returns_none_on_windows_when_command_not_found_at_all():
    with patch("proofkit.diagnostics.platform.system", return_value="Windows"), patch(
        "proofkit.diagnostics.shutil.which", return_value=None
    ):
...
def test_returns_none_on_windows_for_a_real_exe():
    with patch("proofkit.diagnostics.platform.system", return_value="Windows"), patch(
        "proofkit.diagnostics.shutil.which", return_value=r"C:\Python311\python.exe"
    ):
...
def test_returns_hint_for_bat_shim_too():
    with patch("proofkit.diagnostics.platform.system", return_value="Windows"), patch(
        "proofkit.diagnostics.shutil.which", return_value=r"C:\tools\yarn.bat"
    ):
